...
export const useCheckoutMutation = () => {
  ...
  const stripePromise = loadStripe('pk_test_51OZCkbSJAilA93rWtOiZYvAthL2sa9hOQDmaaQKUTAVXku9SGxbQZ6l3IQEHIkUfLOIGWbx9DEDAWjUOmWpeYlYb00j5RmN6UT');
  return useMutation({
    ...
    onSuccess: async (data) => {
      if (data?.data?.app_data?.StatusCode === 6000) {
        ...
        console.log(data?.data?.app_data?.data?.strape.id, "______stra");

        Cookies.set("purchaseId", data?.data?.app_data?.data?.strape.id);
        authorize();
        closeModal();
      }
    },
